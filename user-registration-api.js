...
class UserController {
     ...
     * Receives registration data, hashes the password, and creates an app_user
     * along with their corresponding profile in a single atomic database transaction.
     */
    async createUser(req, res, pool) {
        ...
        const raw_password = registrationData.password;
        const role_id = parseInt(registrationData.role_id, 10);

        // Basic validation
        if (!first_name || !last_name || !email_address || !raw_password || !role_id) {
        ...
        try {
            ...
            // 1. Hash the raw password securely using bcrypt
            const hashed_password = await BCRYPT.hash(raw_password, BCRYPT_SALT_ROUNDS);

            // 2. Insert into the core app_user table (Using Parameterized Queries for SQL Injection protection)
            const insert_user_query = `
                INSERT INTO app_user (f_name, l_name, email, password_hash, role_id)
                VALUES ($1, $2, $3, $4, $5)
                RETURNING user_id;
            `;
            const user_insert_values = [first_name, last_name, email_address, hashed_password, role_id];
            ...
            // 3. Process role-specific profile inserts based on role_id
